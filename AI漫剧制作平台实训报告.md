...
系统核心用例关系如图 2-1 所示，普通用户与管理员围绕模板、生成任务与素材展开交互；"一键生成"用例内部依次触发图片生成、配音与视频合成三个子用例。

![图 2-1 系统用例图](assets/usecase.png)
...
系统采用分层架构，自上而下分为前端层、后端层、AI 服务层与数据层四层，整体架构如图 3-1 所示：

![图 3-1 系统架构图](assets/architecture.png)
...
**核心业务流程数据流向**（模板驱动生成，如图 3-2 所示）：

![图 3-2 核心业务流程数据流向图](assets/flow.png)
...
**关键流程时序图（"AI 图片批量生成"完整流程，如图 3-3 所示）**：

![图 3-3 AI 图片批量生成时序图](assets/sequence.png)
...
系统共 12 张业务表，实体关系如图 5-1 所示：

![图 5-1 数据库 ER 图](assets/er-diagram.png)
...
以下均为实际运行画面截图（图 11-1 ~ 图 11-6），覆盖从首页到成品的完整流程：

![图 11-1 平台首页（模板推荐与一键生成入口）](assets/shot-home.png)

![图 11-2 模板中心（分类筛选与新手友好标记）](assets/shot-template.png)

![图 11-3 模板工作台（必填变量表单与分镜预览）](assets/shot-workspace.png)

![图 11-4 生成任务五步进度（状态机实时回传）](assets/shot-progress.png)

![图 11-5 分镜图片批量生成结果](assets/shot-images.png)

![图 11-6 最终漫剧视频成品（含字幕）](assets/shot-video.png)
