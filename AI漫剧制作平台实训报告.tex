...
系统核心用例关系如图 \ref{fig:usecase} 所示（文字版示意，正式报告以 draw.io/Excalidraw 导出 PNG 嵌入）：
...
系统采用分层架构，自上而下分为四层，如图 \ref{fig:arch} 所示：
...
\textbf{核心业务流程数据流向}（模板驱动生成，如图 \ref{fig:flow}）：
...
\textbf{关键流程时序图（"AI 图片批量生成"完整流程）}，如图 \ref{fig:seq}：
...
文字版 ER 图如图 \ref{fig:er}（正式报告以 draw.io/Excalidraw 导出 PNG 嵌入，标注 1:1 / 1:N / M:N 基数）：
...
\begin{longtable}{p{2.4cm}p{4cm}p{8.6cm}}
\caption{功能演示截图清单}\label{tab:shots}\\
\toprule
图编号 & 标题 & 截图内容\\
\midrule
\endfirsthead
...
图 4-1 & 视频合成产物 & M4 输出 mp4 视频画面（含底部字幕）\\
\bottomrule
\end{longtable}

\textbf{说明}：以上各图编号与标题已规划，正式提交时以实际运行画面截图替换（按要求不得使用设计稿替代）。
